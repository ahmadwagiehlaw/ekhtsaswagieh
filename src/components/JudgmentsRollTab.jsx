      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 flex flex-wrap gap-2 items-center justify-between">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            {['الكل', ...(settings?.sessionTypes || (isSupreme ? ['موضوع', 'فحص'] : ['مرافعة', 'مفوضين']))].map(t => (
              <button
                ...
                className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${sessionTypeFilter === t
        ...
        <div className="flex gap-1.5 flex-wrap relative">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition shadow-sm border ${judgmentFilter === 'all'
                        ...
                        <td className="px-2 py-2 cursor-text" onClick={() => !isEditing && startEdit(cObj, '_role')} title="انقر للتعديل">
                          {isEditing ? (
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${role.includes('طاعن') ? 'bg-rose-100 text-rose-700' :
